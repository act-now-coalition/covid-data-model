...
def _raise_error_if_incorrect_headers(key, headers_list, df):
    ...
    if extra_df_columns or missing_df_columns:
        raise DataExportException(
            key,
            f"Exported headers don't match expected headers. Didn't have headers {missing_df_columns}, had extra columns {extra_df_columns}",
        )
...
def validate_states_df(key, states_df):
    ...
    if len(states_df["Province/State"].unique()) != len(states_df["Province/State"]):
        raise DataExportException(
            key,
            f"Duplicated State Data: {states_df['Province/State'] - states_df['Province/State'].unique()}",
        )
...
def validate_counties_df(key, counties_df):
    ...
    if len(counties_df["State/County FIPS Code"].unique()) != len(
        counties_df["State/County FIPS Code"]
    ):
        raise DataExportException(
            key,
            f"Duplicated County Data: {counties_df[counties_df.duplicated(['State/County FIPS Code'])]['State/County FIPS Code'] }",
        )
