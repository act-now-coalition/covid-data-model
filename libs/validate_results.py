...
def validate_counties_df(key, counties_df, intervention):
    ...
    expected_missing = EXPECTED_MISSING_STATES.union(EXPECTED_MISSING_STATES_FROM_COUNTES)
    ...
    if len(counties_df["County"]) < min_length:
        raise DataExportException(
            key, f"Expected more counties in the output, only found {len(counties_df['County'])}",
        )
