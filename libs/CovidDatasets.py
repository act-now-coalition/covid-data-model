...
def create_local_copy_public_data():
    ...
    _logger.info(
        f"Creating a Local Copy of {PUBLIC_DATA_REPO} at {public_data_local_url}"
    )
...
class Dataset:
    ...
    def backfill_to_init_date(self, series, model_interval):
        ...
        series = series.sort_values(self.DATE_FIELD).reset_index(
            drop=True
        )  # Sort the series by the date of the record
        data_rows = series[
            series[self.CASE_FIELD] > 0
        ]  # Find those rows that actually have reported cases
        interval_rows = data_rows[
            data_rows[self.DATE_FIELD].apply(
                lambda d: (d - self._START_DATE).days % model_interval == 0
            )
        ]
        min_interval_row = interval_rows[
            interval_rows[self.DATE_FIELD] == interval_rows[self.DATE_FIELD].min()
        ].iloc[0]
        ...
        for i in range(0, synthetic_interval):
            ...
            synthetic_row[self.DATE_FIELD] = self._START_DATE + datetime.timedelta(
                days=i
            )
        ...
        return (
            series.append(pd.DataFrame(synthetic_data))
    ...
    def backfill_synthetic_cases(self, series, model_interval):
        ...
        for a in range(0, len(series)):
            ...
            if series.iloc[i][self.CASE_FIELD] == 0:
                series.at[i, self.CASE_FIELD] = self.step_down(
                    i, series, model_interval
                )
        return series
    ...
    def get_all_timeseries(self):
        if self._TIME_SERIES_DATA is None:
            ...
            if self.filter_past_date is not None:
                self._TIME_SERIES_DATA = self._TIME_SERIES_DATA[
                    self._TIME_SERIES_DATA[self.DATE_FIELD] <= self.filter_past_date
                ]
    ...
    def get_raw_timeseries(self):
        raise NotImplementedError(
            "The 'get_raw_timeseries' method must be overriden by the child class"
        )

    def get_all_population(self):
        raise NotImplementedError(
            "The 'get_all_population' method must be overriden by the child class"
        )

    def get_all_beds(self):
        raise NotImplementedError(
            "The 'get_all_beds' method must be overriden by the child class"
        )
    ...
    def combine_state_county_data(self, country, state):
        ...
        county_data = (
            self.get_all_timeseries()[
                ...
                [
                    ...
                    self.CASE_FIELD,
                    self.DEATH_FIELD,
                    self.RECOVERED_FIELD,
                ]
            ]
            .groupby(
                [self.DATE_FIELD, self.COUNTRY_FIELD, self.STATE_FIELD], as_index=False
            )[[self.CASE_FIELD, self.DEATH_FIELD, self.RECOVERED_FIELD]]
            .sum()
        )
        ...
        if len(state_data.index) == 0 and len(county_data.index) == 0:
            ...
            raise Exception(
                "No county or state-level date for {}, {}".format(state, country)
            )

        # Now we fill in whatever gaps we can in the state data using the county data
        curr_date = max(
            state_data[self.DATE_FIELD].max(), county_data[self.DATE_FIELD].max()
        )  # Start on the last date of state data we have
        ...
        while curr_date > self._START_DATE:
            ...
            if len(state_data[state_data[self.DATE_FIELD] == curr_date]) == 0:
                county_data_for_date = copy(
                    county_data[county_data[self.DATE_FIELD] == curr_date]
                )
                if len(county_data_for_date) > 0:
                    ...
                    new_state_row[self.DATE_FIELD] = county_data_for_date[
                        self.DATE_FIELD
                    ]
                    new_state_row[self.CASE_FIELD] = county_data_for_date[
                        self.CASE_FIELD
                    ]
                    new_state_row[self.DEATH_FIELD] = county_data_for_date[
                        self.DEATH_FIELD
                    ]
                    new_state_row[self.RECOVERED_FIELD] = county_data_for_date[
                        self.RECOVERED_FIELD
                    ]
        ...
        return state_data.append(pd.DataFrame(county_data_to_insert)).sort_values(
            self.DATE_FIELD
        )
    ...
    def get_beds_by_country_state(self, country, state):
        ...
        return int(
            round(
                float(matching_beds.iloc[0].at["bedspermille"])
    ...
    def transform_jhu_timeseries(self):
        ...
        while True:
            ...
            try:
                ...
                if e.code == 404:
                    _logger.info(
                        "Received a 404 for date {}. Ending iteration.".format(
                            snapshot_date
                        )
                    )
                    break
                ...
                _logger.info(
                    "File not found for date {}. Ending iteration.".format(
                        snapshot_date
                    )
                )
                break
            ...
            snapshot_date_as_datetime = datetime.datetime.combine(
                snapshot_date, datetime.datetime.min.time()
            )
            ...
            if self.COUNTY_FIELD not in df.columns:
                df[self.COUNTY_FIELD] = (
                    df[self.STATE_FIELD].dropna().apply(parse_county)
                )
            df = df[
                [
                    self.DATE_FIELD,
        ...
        full_report = pd.concat(
            day_reports
        )  # Concat said reports into a single DataFrame
...
class CDSDataset(Dataset):
    ...
    def get_raw_timeseries(self):
        if self._TIME_SERIES_DATA is None:
            self._TIME_SERIES_DATA = pd.read_csv(
                self.timeseries_url, parse_dates=[self.DATE_FIELD]
            )
        return self._TIME_SERIES_DATA
