...
class EnsembleRunner:
    ...
    def __init__(
        ...
        if self.agg_level is AggregationLevel.COUNTY:
            ...
            self.output_file_report = get_run_artifact_path(
                self.fips, RunArtifact.ENSEMBLE_REPORT
            )
            self.output_file_data = get_run_artifact_path(
                self.fips, RunArtifact.ENSEMBLE_RESULT
            )
    ...
    def _load_model_for_fips(self, scenario="inferred"):
        ...
        if scenario == "inferred":
            ...
            eps_final = sp.get_future_suppression_from_r0(
                inferred_params["R0"], scenario=scenario
            )
