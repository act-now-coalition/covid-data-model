...
class SEIRModelAge:
    ...
    def calculate_R0(self, beta=None, S_fracs=None):
        ...
        T_E = np.concatenate(
            [T_E_E, T_E_A, T_E_I, T_E_nonICU, T_E_ICU], axis=1
        )  # all rates to E
        T_A = np.concatenate(
            [T_A_E, T_A_A, T_A_I, T_A_nonICU, T_A_ICU], axis=1
        )  # all rates to A
        T_I = np.concatenate(
            [T_I_E, T_I_A, T_I_I, T_I_nonICU, T_I_ICU], axis=1
        )  # all rates to I
    ...
    def _time_step(self, t, y):
        ...
        dHAdmissions_ICU = sum(
            infected_and_in_hospital_icu
        )  # Ventilators also count as ICU beds.
    ...
    def run(self):
        ...
        S, E, A, I, HGen, HICU, HICUVent = np.split(
            result_time_series[: -self.num_compartments_not_by_age],
        ...
        if self.approximate_R0:
            ...
            Rt = self.calculate_Rt(
                S_fracs_within_age_group, self.suppression_policy(self.t_list)
            )
        ...
        self.results["general_admissions_per_day"] = np.append(
            [0], np.diff(HAdmissions_general)
        )
    ...
    def plot_results(self, y_scale="log", by_age_group=False, xlim=None):
        ...
        if not by_age_group:
            ...
            for ax, n in zip(axes, range(len(self.age_groups))):
                ...
                ax2.set_xlabel("days")
                ax1.set_title(
                    "age group %d-%d" % (self.age_groups[n][0], self.age_groups[n][1])
                )
                ax1.set_yscale("log")
                ax2.set_yscale("log")
                ax1.set_ylim(ymin=1)
