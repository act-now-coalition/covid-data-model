...
class DemographicMapper:
    ...
    def _reconstruct_mortality_inflow_rates(self):
        ...
        mortality_rate_ICU = np.tile(
            self.parameters["mortality_rate_from_ICU"], (self.parameters["t_list"].shape[0], 1)
        ).T
        ...
        mortality_rate_NonICU = np.tile(
            self.parameters["mortality_rate_from_hospital"], (self.parameters["t_list"].shape[0], 1)
        ).T
                ...
                for measure_unit_name in predictions[measure_name]:
                    ...
                    if self.risk_modifier_by_age is not None:
                        if measure_name in self.risk_modifier_by_age:
                            modified_weights = weights * self.risk_modifier_by_age[measure_name](
                                age_bin_centers
                            )
                            modified_weights /= modified_weights.sum()
                    mapped_predictions[measure_name][measure_unit_name] = predictions[measure_name][
                        measure_unit_name
                    ].dot(modified_weights)
