...
class WebUIDataAdaptorV1:
    ...
    def backfill_output_model_fips(self, fips, t0, final_beds, output_model):
        ...
        intervals_to_backfill = math.ceil(
            (t0.date() - backfill_to_date).days / self.output_interval_days
        )
        backfill_offsets = range(
            -intervals_to_backfill * self.output_interval_days,
            0,
        ...
        if len(fips) == 5:
            actual_timeseries = self.county_timeseries[
                (self.county_timeseries["fips"] == fips)
            ]
        else:
            actual_timeseries = self.state_timeseries[
                (self.state_timeseries["state"] == self.state_abbreviation)
            ]

        backfill = pd.merge(
            backfill,
            actual_timeseries[["date", "cases", "deaths"]],
            on="date",
            how="left",
        )
        ...
        backfill["all_infected"] = (
            output_model["all_infected"][0]
            * backfill["cases"]
            / backfill["cases"].max()
        ).fillna(0)
        backfill["all_hospitalized"] = np.multiply(
            backfill["all_infected"], hospitalization_rate
        ).fillna(0)
    ...
    def map_fips(self, fips):
        ...
        for i_policy, suppression_policy in enumerate(policies):
            if (
                suppression_policy == "suppression_policy__full_containment"
            ):  # No longer shipping this.
                continue
            ...
            # Col 9
            output_model["all_hospitalized"] = np.add(
            ...
            # Col 12
            final_beds = np.mean(output_for_policy["HGen"]["capacity"]) + np.mean(
            ...
            if suppression_policy != "suppression_policy__inferred":
                ...
                output_model = pd.concat([backfill, output_model])[
                    output_model.columns
            ...
            if len(fips) == 5:
                output_path = os.path.join(
                    output_dir, f"{self.state_abbreviation}.{fips}.{i_policy}.json"
                )
            else:
                output_path = os.path.join(
                    output_dir, f"{self.state_abbreviation}.{i_policy}.json"
                )
    ...
    def generate_state(self, states_only=False):
        ...
        if not states_only:
            ...
            if not self.include_imputed:
                ...
                fips_with_cases = (
                    fips_with_cases[fips_with_cases.cases > 0].fips.unique().tolist()
                )
