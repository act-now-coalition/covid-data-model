...
class Backtester:
    ...
    def load_observations(fips=None, ref_date=REF_DATE):
        ...
        observations = pd.DataFrame(
            observations, index=pd.DatetimeIndex(observation_dates)
    ...
    def run_model_fitter_for_backtest(self, observation_days_blinded):
        ...
        mf.observed_new_cases = self.observations["new_cases"].values[
            :-observation_days_blinded
        ]
        mf.observed_new_deaths = self.observations["new_deaths"].values[
            :-observation_days_blinded
        ]
        ...
        (
            ...
            mf.hosp_stdev,
            mf.deaths_stdev,
        ) = mf.calculate_observation_errors()
        ...
        if mf.hospitalization_data_type is not None:
            if (
                ...
                predicted_hosp = (
                    mf.mle_model.results["HGen"] + mf.mle_model.results["HICU"]
                )
    ...
    def run_backtest(self):
        ...
        for d in np.arange(
            1, self.max_observation_days_blinded + 1, self.num_days_per_step
        ):
            ...
            for observation_type in ["new_cases", "new_deaths", "hospitalizations"]:
                if observation_type in self.observations:
                    for error_type in self.error_types:
                        ...
                        backtest_record["error_type"].extend(
                            [error_type] * error.shape[0]
                        )
            ...
            for error_type in backtest_results.error_type.unique():
                ...
                if error_type not in ["rmse", "nrmse"]:
                    fig, axes = plt.subplots(
                        nrows=int(np.ceil(df.days_of_forecast.max() / 3)),
                        ncols=3,
                        figsize=(18, 12),
                    )
                    for d, ax in list(
                        zip(df.days_of_forecast.unique(), np.ravel(axes))
                    ):
                        df[df.days_of_forecast == d].plot(
                            "observation_end_date", "error", ax=ax
                        )
                    ...
                    plt.suptitle(
                        f"{observation_type}\n{error_type_to_meaning(error_type)}",
                        fontsize=15,
                    )
                else:
                    plt.figure()
    ...
    def plot_historical_predictions(
        self, historical_predictions=None, observations=None, pdf=None
    ):
        """
    ...
    def generate_report(self):
        ...
        self.plot_historical_predictions(
            self.historical_predictions, self.observations, pdf
        )
        pdf.close()
