...
def cache_hospital_beds():
    ...
    url = (
        "http://opendata.arcgis.com/datasets/f3f76281647f4fbb8a0d20ef13b650ca_0.geojson"
    )
...
def load_ensemble_results(fips, run_mode="default"):
    ...
    if len(fips) == 5:  # County
        ...
        state, county = (
            county_metadata.loc[fips]["state"],
            county_metadata.loc[fips]["county"],
        )
        path = os.path.join(
            OUTPUT_DIR,
            "pyseir",
            state,
            "data",
            f"{state}__{county}__{fips}__{run_mode}__ensemble_projections.json",
        )
    elif len(fips) == 2:
        ...
        path = os.path.join(
            ...
            state,
            "data",
            f"{state}__{fips}__{run_mode}__ensemble_projections.json",
        )
...
def load_county_metadata_by_fips(fips):
    ...
    county_metadata_merged = (
        county_metadata.merge(hospital_bed_data, on="fips", how="left")
        .set_index("fips")
        .loc[fips]
        .to_dict()
    )
...
def load_new_case_data_by_state(state, t0):
    ...
    state_case_data = _state_case_data[
        _state_case_data["state"] == us.states.lookup(state).abbr
    ]
    times_new = (state_case_data["date"] - t0).dt.days.iloc[1:]
    observed_new_cases = (
        state_case_data["cases"].values[1:] - state_case_data["cases"].values[:-1]
    )
    observed_new_deaths = (
        state_case_data["deaths"].values[1:] - state_case_data["deaths"].values[:-1]
    )

    _, filter_idx = hampel_filter__low_outliers_only(
        observed_new_cases, window_size=5, n_sigmas=2
    )
