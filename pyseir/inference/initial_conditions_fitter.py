...
class InitialConditionsFitter:
    ...
    def plot_fit(self):
        ...
        plt.text(
            0.1,
            0.8,
            "$C(t) = %1.3f\ \exp^{(t - %1.3f) / %1.3f}$"
            % (
                self.model_params["norm"],
        ...
        plt.text(
            0.1,
            0.7,
            f"{self.t0_case_count} Cases on {self.t0_date.date()}",
            transform=plt.gca().transAxes,
            fontsize=14,
        )
        plt.text(
            0.1,
            0.6,
            f"$\chi^2 /d.o.f.=%1.3f$" % self.reduced_chi2,
            transform=plt.gca().transAxes,
            fontsize=14,
        )
...
def _fit_fips(fips, generate_report=False):
    ...
    try:
        fitter = InitialConditionsFitter(
            fips=fips, t0_case_count=1, start_days_before_t0=0, start_days_after_t0=1000
        )
        ...
        if generate_report:
            ...
            plt.savefig(
                os.path.join(
                    state_dir,
                    "reports",
                    f"{fitter.state}__{fitter.county}__{fitter.fips}__t0_fit.pdf",
                ),
                bbox_inches="tight",
            )
            plt.close()
...
def generate_start_times_for_state(state, generate_report=False):
    ...
    merged["days_from_2020_01_01"] = (
        merged.t0_date - datetime.fromisoformat("2020-01-01")
    ).dt.days
    ...
    if samples_with_no_data.any():
        ...
        for estimator in [LinearRegression(), RandomForestRegressor(), BayesianRidge()]:
            ...
            logging.info(
                f'{estimator.__class__.__name__} CV r2: {cv_result["test_score"].mean()}'
            )
        ...
        merged.loc[samples_with_no_data, "days_from_2020_01_01"] = best_model.predict(
            X_predict
        )
