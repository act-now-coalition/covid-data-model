...
class ModelFitter:
    ...
    def calculate_observation_errors(self):
        ...
        cases_stdev[self.observed_new_cases <= 4] = (
            cases_stdev[self.observed_new_cases <= 4] * 3
        )
    ...
    def run_model(self, R0, eps, t_break, log10_I_initial):
        ...
        self.SEIR_kwargs["E_initial"] = (
            self.steady_state_exposed_to_infected_ratio
            * 10 ** log10_I_initial
            * age_distribution
        )
    ...
    def _fit_seir(
        ...
        if self.observed_new_deaths.sum() > self.min_deaths:
            chi2_deaths = calc_chi_sq(
                self.observed_new_deaths, predicted_deaths, self.deaths_stdev
            )
        else:
            chi2_deaths = 0
    ...
    def plot_fitting_results(self):
        ...
        plt.savefig(
            output_file.replace("mle_fit_results", "mle_fit_model"), bbox_inches="tight"
        )
        plt.close()
        ...
        try:
            ...
            while retries_left > 0 and model_is_empty:
                ...
                try:
                    model_fitter.fit()
                    if (
                        model_fitter.mle_model
                        and os.environ.get("PYSEIR_PLOT_RESULTS") == "True"
                    ):
    ...
    if not states_only:
        ...
        if len(all_fips) > 0:
            ...
            for fips, fitter in zip(all_fips, fitters):
                if fitter:
                    with open(
                        get_run_artifact_path(fips, RunArtifact.MLE_FIT_MODEL), "wb"
                    ) as f:
