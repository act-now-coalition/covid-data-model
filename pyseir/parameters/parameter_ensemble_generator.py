...
class ParameterEnsembleGenerator:
    ...
    def __init__(self, fips, N_samples, t_list, I_initial=1, suppression_policy=None):
        ...
        self.fips = fips
        self.agg_level = (
            AggregationLevel.COUNTY if len(self.fips) == 5 else AggregationLevel.STATE
        )
        ...
        if self.agg_level is AggregationLevel.COUNTY:
            ...
            self.population = population_data.get_county_level(
                "USA", state=self.state_abbr, fips=self.fips
            )
            ...
            self.icu_beds = (
                beds_data.get_county_level(
                    self.state_abbr, fips=self.fips, column="icu_beds"
                )
                or 0
            )
        else:
            self.state_abbr = us.states.lookup(fips).abbr
            self.population = population_data.get_state_level(
                "USA", state=self.state_abbr
            )
            self.beds = beds_data.get_state_level(self.state_abbr) or 0
            self.icu_beds = (
                beds_data.get_state_level(self.state_abbr, column="icu_beds") or 0
            )
            ...
            parameter_sets.append(
                dict(
                    ...
                    fraction_icu_requiring_ventilator=max(
                        np.random.normal(loc=0.6, scale=0.1), 0
                    ),
                    sigma=1
                    / np.random.normal(
                        loc=3.0, scale=0.86
                    ...
                    symptoms_to_mortality_days=np.random.normal(
                        loc=18.8, scale=0.45
                    ),  # Imperial College
                    hospitalization_length_of_stay_general=np.random.normal(
                        loc=6, scale=1
                    ),
                    hospitalization_length_of_stay_icu=np.random.normal(
                        loc=14, scale=3
                    ),
                    ...
                    beds_general=self.beds
                    * 0.4
                    * 2.07,  # 60% utliization, no scaling...
                    # TODO.. Patch this After Issue 132
                    beds_ICU=(1 - 0.85)
                    * self.icu_beds,  # No scaling, 85% utilization...
