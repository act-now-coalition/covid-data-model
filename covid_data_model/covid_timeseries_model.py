...
class CovidTimeseriesModel:
    ...
    def __init__(self, model_parameters):
        self.model_parameters = self._initialize_parameters(model_parameters)
    ...
    def _initialize_parameters(self, parameters):
        ...
        init_data_cycle = parameters["timeseries"].sort_values("date").iloc[0]
        parameters["init_date"] = init_data_cycle["date"]
        # Get the latest date in the data
        parameters["last_date"] = (
            parameters["timeseries"].sort_values("date")["date"].iloc[-1]
        )
        # Calculate the number of iterations needed to cover the data
        parameters["data_iterations"] = math.floor(
            # timedelta subtraction is not inclusive, but we need it to be, so we'll add one day
            (
                parameters["last_date"]
                - (parameters["init_date"] - datetime.timedelta(days=1))
            )
            / datetime.timedelta(days=parameters["model_interval"])
        )

        # Sum the data and projection interations for a single number of iterations to cycle the model
        parameters["total_iterations"] = (
            parameters["data_iterations"] + parameters["projection_iterations"]
        )
        parameters["original_available_hospital_beds"] = parameters["beds"] * (
            1 - parameters["initial_hospital_bed_utilization"]
        )
    ...
    def calculate_r(self, current_cycle, previous_cycle):
        ...
        return self.model_parameters["r0"]

    def calculate_effective_r(self, current_cycle, previous_cycle):
        # In order to account for interventions, we need to allow effective r to be changed manually.
        if self.model_parameters["interventions"] is not None:
            ...
            for d in sorted(self.model_parameters["interventions"].keys())[::-1]:
                if (
                    current_cycle["date"] >= d
                ):  # If the current cycle occurs on or after an intervention
                    return self.model_parameters["interventions"][d] * (
                        previous_cycle["ending_susceptible"]
                        / self.model_parameters["population"]
                    )
        # Calculate effective r by accounting for herd-immunity
        return current_cycle["r"] * (
            previous_cycle["ending_susceptible"] / self.model_parameters["population"]
        )

    def calculate_newly_infected(self, current_cycle, previous_cycle):
        ...
        if current_cycle["cases"] is not None:
            return (
                ...
                / self.model_parameters["hospitalization_rate"]
            )
        return 0
    ...
    def calculate_predicted_hospitalized(self, current_cycle):
        return (
            current_cycle["newly_infected"]
            * self.model_parameters["hospitalization_rate"]
        )

    def calculate_ending_susceptible(self, current_cycle, previous_cycle):
        # Number of people who haven't been sick yet
        return self.model_parameters["population"] - (
            current_cycle["newly_infected"]
    ...
    def make_cycle(self, i):
        # Check if the date we are trying to model is within the data we have
        timeseries = self.model_parameters["timeseries"]
        current_cycle_date = self.model_parameters["init_date"] + datetime.timedelta(
            days=self.model_parameters["model_interval"] * i
        )
        ...
        if any(dates.isin([current_cycle_date])):
            ...
            return timeseries[timeseries["date"] == current_cycle_date].to_dict(
                "records"
            )[0]
        else:
    ...
    def build_init_cycle(self, init_data_cycle):
        ...
        init_cycle = {
            ...
            "date": self.model_parameters["init_date"],
            "r": self.model_parameters["r0"],
            "effective_r": None,
            "cases": init_data_cycle["cases"],
            "actual_reported": init_data_cycle["cases"],
            "current_infected": 0,
            "est_actual_chance_of_infection": 0,
            "newly_infected": init_data_cycle["cases"]
            / self.model_parameters["hospitalization_rate"],
            "currently_infected": 0,
            "cumulative_infected": None,
            "cumulative_deaths": None,
            "recovered_or_died": 0,
            "ending_susceptible": self.model_parameters["population"]
            - (
                init_data_cycle["cases"] / self.model_parameters["hospitalization_rate"]
            ),
            "predicted_hospitalized": 0,
            "available_hospital_beds": self.model_parameters[
                "original_available_hospital_beds"
            ],
        }
        return init_cycle
    ...
    def iterate_model(self):
        ...
        init_data_cycle = (
            self.model_parameters["timeseries"].sort_values("date").iloc[0]
        )
        cycle_series = [self.build_init_cycle(init_data_cycle)]

        previous_cycle = cycle_series[0]
        for i in range(self.model_parameters["total_iterations"]):
            # Step through existing empirical data
            current_cycle = self.make_cycle(i)
            current_cycle["i"] = i  # Note the cycle
            logging.debug("Calculating values for {}".format(current_cycle["date"]))
            # Calculate the r0 value
            r = self.calculate_r(current_cycle, previous_cycle)
            current_cycle["r"] = r
            eff_r = self.calculate_effective_r(current_cycle, previous_cycle)
            current_cycle["effective_r"] = eff_r
            # Calculate the number of newly infected cases
            ni = self.calculate_newly_infected(current_cycle, previous_cycle)
            ...
            curr_i = self.calculate_currently_infected(
                cycle_series,
                self.model_parameters["rolling_intervals_for_current_infected"],
            )
            ...
            cum_d = self.calculate_cumulative_deaths(
                current_cycle,
                previous_cycle,
                self.model_parameters["case_fatality_rate"],
                self.model_parameters["case_fatality_rate_hospitals_overwhelmed"],
            )
            ...
            ahb = self.calculate_available_hospital_beds(
                current_cycle,
                previous_cycle,
                self.model_parameters["max_hospital_capacity_factor"],
                self.model_parameters["original_available_hospital_beds"],
                self.model_parameters["hospital_capacity_change_daily_rate"],
            )
    ...
    def forecast_region(self):
        cycle_series = self.iterate_model()
        data = {
            ...
            "Est. Actual Chance of Inf.": [
                s["est_actual_chance_of_infection"] for s in cycle_series
            ],
