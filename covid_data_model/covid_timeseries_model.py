...
class CovidTimeseriesModel:
    ...
    def make_cycle(self, i, model_parameters):
        ...
        current_cycle_date = model_parameters["init_date"] + datetime.timedelta(
            days=model_parameters["model_interval"] * i
        )
        ...
        if any(dates.isin([current_cycle_date])):
            ...
            return model_parameters["timeseries"][
                model_parameters["timeseries"]["date"] == current_cycle_date
            ].to_dict("records")[0]
        else:
    ...
    def build_init_cycle(self, model_parameters, init_data_cycle):
        ...
        init_cycle = {
            ...
            "available_hospital_beds": model_parameters[
                "original_available_hospital_beds"
            ],
        }
        return init_cycle
    ...
    def initialize_parameters(self, model_parameters):
        ...
        model_parameters["last_date"] = (
            model_parameters["timeseries"].sort_values("date")["date"].iloc[-1]
        )
        ...
        model_parameters["data_iterations"] = math.floor(
            ...
            (
                model_parameters["last_date"]
                - (model_parameters["init_date"] - datetime.timedelta(days=1))
            )
        ...
        model_parameters["total_iterations"] = (
            model_parameters["data_iterations"]
            + model_parameters["projection_iterations"]
        )
        model_parameters["original_available_hospital_beds"] = model_parameters[
            "beds"
        ...
        return (
            [self.build_init_cycle(model_parameters, init_data_cycle)],
            model_parameters,
        )
    ...
    def iterate_model(self, model_parameters):
        ...
        for i in range(1, model_parameters["total_iterations"]):
            ...
            current_cycle["r"] = r
            eff_r = self.calculate_effective_r(
                current_cycle, previous_cycle, model_parameters
            )
            ...
            curr_i = self.calculate_currently_infected(
                cycle_series, model_parameters["rolling_intervals_for_current_infected"]
            )
            ...
            ph = self.calculate_predicted_hospitalized(current_cycle, model_parameters)
            ...
            es = self.calculate_ending_susceptible(
                current_cycle, previous_cycle, model_parameters
            )
