...
model_interval = 4
rolling_intervals_for_current_infected = math.floor(
    length_of_average_infection / model_interval
)

max_search_intervals_for_synthesis = 10
compounding_factor_for_synthesis_per_interval = 0.5
...
def get_snapshot(full_timeseries, date, state, country):
    ...
    filtered_timeseries = full_timeseries[
        (full_timeseries["state"] == state)
        & (full_timeseries["country"] == country)
        & (full_timeseries["date"] == date.strftime("%Y-%m-%d"))
        & (full_timeseries["county"].isna())
    ]
    ...
    for i in range(1, max_search_intervals_for_synthesis + 1):
        ...
        logging.debug(
            "Checking cases for date {}: {}".format(
                future_date, future_snapshot["confirmed"]
            )
        )
...
def forecast_region(
    ...
    while True:
        ...
        if previous_newly_infected > 0:
            ...
            newly_infected = (
                previous_newly_infected
                * effective_r0
                * previous_ending_susceptible
                / pop
            )
            logging.debug(
                "{} = {} * {} * {} / {}".format(
        ...
        if available_hospital_beds > predicted_hospitalized:
            cumulative_deaths += int(round(newly_infected * case_fatality_rate))
        else:
            if overwhelmed_date is None:
                overwhelmed_date = snapshot_date
                logging.info(
                    "Hospitals in {} overwhelmed on {}".format(state, snapshot_date)
                )
        ...
        if snapshot["confirmed"] is not None:
            est_actual_chance_of_infection = (
                snapshot["confirmed"] / hospitalization_rate * 2
            ) / pop
            actual_reported = round(snapshot["confirmed"], 2)

        ending_susceptible = int(
            round(pop - newly_infected - previously_infected - recovered_or_died)
        )
